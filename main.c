...
void usage ()
{
    fprintf (stderr, "\nUsage: pfm_recompute [-1|-2|-3|-4|-c|-f -z] <PFM_HANDLE_FILE or PFM_LIST_FILE>\n");
    fprintf (stderr, "\nWhere:\n\n");
...
{
    int32_t             i, j, m, percent = 0, old_percent = -1, pfm_handle, flag, recnum;
    PFM_OPEN_ARGS       open_args;
    float               total, null_depth, minz, maxz;
    BIN_RECORD          bin_record;
    DEPTH_RECORD        *depth;
    NV_I32_COORD2       coord;
    uint8_t             clear, filter, limit;
    char                c;
    extern char         *optarg;
    extern int          optind;
        ...
        for (j = 0; j < open_args.head.bin_width; j++)
        {
            ...
            if (clear)
            {
                    ...
                    &recnum))
                {
                    for (m = 0 ; m < recnum ; m++)
                    {
                        ...
                        {
                            if (!limit || (depth[m].xyz.z >= minz && depth[m].xyz.z <= maxz))
                            {
                                if (filter)
                                {
                                    depth[m].validity &= ~(PFM_FILTER_INVAL | PFM_SELECTED);
                                }
                                else
                                {
                                    depth[m].validity &= ~(PFM_INVAL | PFM_SELECTED);
                                }
                                depth[m].validity |= PFM_MODIFIED;

                                update_depth_record_index (pfm_handle, &depth[m]);
                            }
                        }
                    }
                    ...
                    recompute_bin_values_index (pfm_handle, coord, &bin_record, PFM_CHECKED);
                }
            }
            else
            {
            ...
            if (bin_record.num_soundings)
            {
                ...
                if (bin_record.max_depth < null_depth)
                {
                    ...
                    {
                        if (bin_record.min_filtered_depth < open_args.head.min_filtered_depth)
                        {
                            open_args.head.min_filtered_depth = bin_record.min_filtered_depth;
                            open_args.head.min_filtered_coord = bin_record.coord;
                        }
                    
                        if (bin_record.max_filtered_depth > open_args.head.max_filtered_depth)
                        {
                            open_args.head.max_filtered_depth = bin_record.max_filtered_depth;
                            open_args.head.max_filtered_coord = bin_record.coord;
                        }

                        if (bin_record.standard_dev < open_args.head.min_standard_dev)
                        {
                            open_args.head.min_standard_dev = bin_record.standard_dev;
                        }

                        if (bin_record.standard_dev > open_args.head.max_standard_dev)
                        {
                            open_args.head.max_standard_dev = bin_record.standard_dev;
                        }
                    }
